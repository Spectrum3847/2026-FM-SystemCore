...
- **Alliance**: everything (drive direction, aim and shot targets, field flipping, PathPlanner's
  flip, the auto preview) asks [AllianceSource](src/main/java/frc/spectrumLib/util/AllianceSource.java),
  never `MatchState.getAlliance()`. Order: the Pre-Match tab's **Alliance Override** chooser (Auto /
  Blue / Red, default Auto), then what the DS reports now, then the last alliance it reported this
  boot, then blue, so the robot always drives. Anything but the DS raises a warning; FMS attached
  with no alliance for 2 s raises an error. Logged as `Alliance/InUse` and `Alliance/Source`; the
  Match tab shows `Alliance/Display`.
