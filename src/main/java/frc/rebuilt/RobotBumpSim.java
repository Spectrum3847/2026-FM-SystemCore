 ...
 * ChassisVelocities fieldRelativeSpeeds = mapleSimSwerveDrivetrain.mapleSimDrive.getDriveTrainSimulatedChassisVelocitiesFieldRelative();
 *
...
public class RobotBumpSim {
     ...
     * The companion {@link #BUMP_LINE_ENDS} array stores the end point including the maximum Y.
     *
     ...
     * physically accurate visual position. The caller must also call {@link
     * #getSimWorldPose(Pose2d)} and apply it to MapleSim so the actual simulation position matches
     * (see the class-level Javadoc for a complete integration example).
     *
     ...
     * @param subticks Physics sub-steps per period. Must match the {@code subticks} value used by
     *     any companion ball/object sim so they stay in sync. Typical value: 5 (= 4 ms sub-steps
     *     per 20 ms loop).
     * @return A {@link Pose3d} with physically correct X, Z, pitch, and roll.
     */
    public Pose3d update(Pose2d robotPose2d, ChassisVelocities fieldRelativeSpeeds, int subticks) {
        double vx = fieldRelativeSpeeds.vx;
