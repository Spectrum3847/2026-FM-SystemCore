...
public class SpectrumCANcoder {
    ...
    public SpectrumCANcoder(
        ...
        if (config.isAttached()) {
            ...
            // Retried at boot, then in the background until it applies (CanConfigRetry).
            CanConfigRetry.Entry entry =
                    CanConfigRetry.INSTANCE.applyAtBoot(
                            "CANcoder " + CANcoderID,
                            canCoder,
                            CANcoderID,
                            motor.getNetwork().getName(),
                            CanConfigRetry.CONFIG,
                            true,
                            timeout -> canCoder.getConfigurator().apply(canCoderConfigs, timeout));
            if (canCoderResponseOK(entry.isApplied() ? StatusCode.OK : StatusCode.ConfigFailed)) {
    ...
    public SpectrumCANcoder modifyMotorConfig(TalonFX motor, Config mechConfig) {
        ...
        TalonFXFactory.applyConfigAtBoot(mechConfig.getName(), motor, talonConfigMod);
