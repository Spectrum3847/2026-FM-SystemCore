 ...
 * telemetry, and convenience {@link org.wpilib.command2.Command} factories.
 *
 ...
 *   <li>Convenience {@link org.wpilib.command2.button.Trigger} factories (at/above/below
 *       thresholds)
...
public abstract class Mechanism implements Subsystem {
    ...
    Alert currentAlert = new Alert("", Level.MEDIUM);
    ...
    /**
     * This loop's motor readings as an AdvantageKit input. Every getter reads from here, so a
     * replayed log drives the mechanism's logic exactly as the field did. See {@link MotorInputs}.
     */
    @Getter private final MotorInputs inputs = new MotorInputs();

    /** Log table name for {@link #inputs}, built once. */
    ...
    protected Mechanism(Config config) {
        ...
        if (isAttached()) {
            ...
            inputs.followerSupplyCurrentAmps = new double[followerSupplySignals.length];
            inputs.followerConnected = new boolean[followerSupplySignals.length];
    ...
    public void logBatteryUsage() {
        if (isAttached()) {
            ...
            for (int i = 0; i < followerMotors.length; i++) {
                double amps = inputs.followerSupplyCurrentAmps[i];
     ...
     */
    protected void refreshSignalsOncePerLoop() {
        long loop = RobotLoop.count();
        if (loop == lastSignalRefreshLoop || loopSignals.length == 0) {
            return;
        }
        lastSignalRefreshLoop = loop;
        StatusCode status = BaseStatusSignal.refreshAll(loopSignals);
        inputs.connected = status.isOK() && motor.isConnected();
        inputs.positionRotations = positionSignal.getValueAsDouble();
        inputs.latencyCompensatedPositionRotations =
    ...
    /**
     * Refreshes the motor inputs for this loop. Called from {@link #periodic()} so the inputs are
     * recorded every loop even when nothing reads them; subclasses that override {@code periodic()}
     * get the same from their first getter call.
     */
    ...
    /** This loop's value of one of the inputs, or 0 when there is no hardware. */
    private double readInput(DoubleSupplier field) {
        if (!config.attached) {
            return 0;
        }
        refreshSignalsOncePerLoop();
    ...
    public double getStatorCurrent() {
        return readInput(() -> inputs.statorCurrentAmps);
    }
    ...
    public double getSupplyCurrent() {
        return readInput(() -> inputs.supplyCurrentAmps);
    }
    ...
    public double getVoltage() {
        return readInput(() -> inputs.voltage);
    }
    ...
    public double getTemp() {
        return readInput(() -> inputs.tempCelsius);
    }
    ...
    public double getPositionRotations() {
        return readInput(() -> inputs.positionRotations);
    }
    ...
    public double getLatencyCompensatedPositionRotations() {
        ...
        refreshSignalsOncePerLoop();
        return inputs.latencyCompensatedPositionRotations;
    }
    ...
    public double getVelocityRPM() {
        return Conversions.RPStoRPM(readInput(() -> inputs.velocityRps));
    }
    ...
    public static class Config {
        ...
        public void configGravityType(int slot, boolean isArm) {
            ...
            if (slot == 0) {
                ...
                DriverStationErrors.reportWarning("MechConfig: Invalid slot", false);
            }
        }
        ...
        private void talonConfigFeedForward(int slot, double kV, double kA, double kS, double kG) {
            if (slot == 0) {
                ...
                DriverStationErrors.reportWarning("MechConfig: Invalid FeedForward slot", false);
            }
        }
        ...
        private void talonConfigFeedbackPID(int slot, double kP, double kI, double kD) {
            if (slot == 0) {
                ...
                DriverStationErrors.reportWarning("MechConfig: Invalid Feedback slot", false);
            }
        }
    }
}
